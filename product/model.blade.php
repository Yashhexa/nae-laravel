...
@media only screen and (max-width: 700px){
  .modal-content {
            ...
            <div class="container-fluid" style="background-color: #fff; padding: 11px;">
                <div class="row">
                    <div class="col-lg-6 order-lg-2 order-1">
                        ...
                        <model-viewer camera-controls alt="{{$product->sku}} - {!! $product->descrption !!}" class="image_selected" style="margin-bottom: 10px;" src="/models/{{$product->model_url}}">
                        </model-viewer>                            
                        @endif                        
                        <div class="image_selected"><img src="{{ $product->image_url }}"
                                alt="{{$product->sku}} - {!! $product->descrption !!}" id="main_image"></div>
                    <div class="row">
                        <div class="col-md-2 col-4 p-3 cursors"><img src="{{ $product->image_url }}" data-image-path="{{ $product->image_url }}" onclick='changeImage(this)'
                                    alt="{{$product->type}}" class="img-fluid img-thumbnail"></div>
                       
                        @foreach ($photos as $photo)
                        <div class="col-md-2 col-4 p-3 cursors"><img src="{{ $photo->photo_url }}" data-image-path="{{ $photo->photo_url }}" onclick='changeImage(this)'
                        ...
                        <div class="product_description">
                            <nav>
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="/housings/{{$category->id}}">{{ $category->name }}</a></li>
                            ...
                            <div class="row">
                                <div class="col-md-6"> 
                                    @if($product->status == "Pre-Release")
                                    <span class="product_price text-secondary">$65.45</span> for one 3-D printed sample.
                                    @else
                                    <span class="product_price text-secondary">${{ $product->msrp250 }}</span> each (min. 250pc).
                                    @endif
                            </div>
                            ...
                            <div class="row"><form action="{{route('create_cart')}}" method="GET">
                                <div class="col-xs-6 mb-4">
                                    <div class="form-floating">
                                        <select name="quantity" class="form-select" onchange="updateUrl(this.value)">
                                            <option value="250">
                                                250pc - ${{ number_format($product->msrp250 * 250, 2, '.', ',') }}
                                            </option>
                                            <option value="500">
                                                500pc - ${{ number_format($product->msrp500 * 500, 2, '.', ',') }}
                                            </option>
                                            <option value="1000">
                                                1,000pc - ${{ number_format($product->msrp1000 * 1000, 2, '.', ',') }}
                                            </option>
                                            <option value="2500">
                                                2,500pc - ${{ number_format($product->msrp2500 * 2500, 2, '.', ',') }}
                                            </option>
                                            <option value="5000">
                                                5,000pc - ${{ number_format($product->msrp5000 * 5000, 2, '.', ',') }}
                                            </option>
                                            <option value="10000">
                                                10,000pc - ${{ number_format($product->msrp10000 * 10000, 2, '.', ',') }}
                                            </option>
                                        </select>
                                ...
                                @endphp
                                <div class="col-xs-6"><input type="hidden" name="product_id" value="{{$product->id}}" /> <button
                                        type="submit" class="btn btn-success btn-lg"><i
                                            class="bi bi-cart-check-fill"></i> Add to cart</button>
                                            @if(auth()->check())
                                                <a href="/3d-configurator/frontend/?productId={{ $product->id }}&userId={{ $userId }}" id="customizedUrl" class="btn btn-primary btn-lg">Customize this enclosure</a>
                                            @else
                                                <a href="/3d-configurator/frontend/?productId={{ $product->id }}" id="customizedUrl" class="btn btn-primary btn-lg">Customize this enclosure</a>
                                            @endif
                                            <!-- <a href="/3d-configurator/frontend/?productId={{$product->id}}" class="btn btn-primary btn-lg"><i
    ...
    <h2 class="fw-light">Check out more from the <span class="text-success">{{ $category->name }}</span> category:</h2>
  ...
  <div class="card-body">
    ...
    <a href="/enclosures/{{$product->url_key}}" class="btn btn-success">View Enclosure</a>
    <a href="/customize/{{$product->url_key}}" class="btn btn-primary">Customize</a>
    </div>
  </div>
</div>
                    </div>
...
<div class="card" >
  ...
  <div class="card-body">
    ...
    <a href="/customize/{{$item->id}}" class="btn btn-primary">Customize</a>
    </div>
  </div>
</div>
                    </div>
